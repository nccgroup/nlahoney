...
{
	WLog_INFO(TAG,"[HONEY] ntlm_compute_ntlm_v2_hash");
	SSPI_CREDENTIALS* credentials = context->credentials;
#ifdef WITH_DEBUG_NTLM
	...
	{
		WLog_INFO(TAG,"[HONEY] -- Hash call back ");
		int ret;
	...
	{
		WLog_INFO(TAG,"[HONEY] -- Using SAM");
		int ret = ntlm_fetch_ntlm_v2_hash(context, hash);
		WLog_INFO(TAG,"[HONEY] RET ntlm_compute_ntlm_v2_hash");
		return ret;
	}
...
{
	WLog_INFO(TAG,"[HONEY] ntlm_compute_message_integrity_check");
	WLog_INFO(TAG,"[HONEY] -- NEGOTIATE_MESSAGE    - context->NegotiateMessage    - (we receive)");
	...
	if (winpr_HMAC_Init(hmac, WINPR_MD_MD5, context->ExportedSessionKey, WINPR_MD5_DIGEST_LENGTH)) // ExportedSessionKey is RandomSessionKey (which we can set to all zeros)
	{
		winpr_HMAC_Update(hmac, (BYTE*)context->NegotiateMessage.pvBuffer, // this is from the client
		                  context->NegotiateMessage.cbBuffer);
		winpr_HMAC_Update(hmac, (BYTE*)context->ChallengeMessage.pvBuffer, // this is from us
