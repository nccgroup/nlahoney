...
SECURITY_STATUS ntlm_read_NegotiateMessage(NTLM_CONTEXT* context, PSecBuffer buffer)
{
	...
	{
		WLog_INFO(TAG,"[HONEY] -- no workskation");
		Stream_Free(s, FALSE);
		return SEC_E_INVALID_TOKEN;
	} else {
		WLog_INFO(TAG,"[HONEY] -- workskation %s", message->Workstation);
	}
...
SECURITY_STATUS ntlm_read_AuthenticateMessage(NTLM_CONTEXT* context, PSecBuffer buffer)
{
	...
	if (ntlm_read_message_fields(s, &(message->Workstation)) < 0) /* WorkstationFields (8 bytes) */
		goto fail;
		WLog_INFO(TAG,"[HONEY] -- workstation %s",&(message->Workstation) );
